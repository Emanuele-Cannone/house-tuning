  ...
  switch (action.type) {
    case "update":
      return {
        ...
        description: <input type="text" placeholder="Veicolo" className="input input-bordered w-full max-w-xs mt-4" />,
...
function Index({ vehicles, auth }: Readonly<IndexProps>) {
  ...
  return (
    ...
    >
      ...
      <Modal modalId={modal.idModal} description={modal.description} title={modal.title} confirm={() => modal.confirmButton()} cancel={() => closeModal()} />
    </AuthenticatedLayout>
